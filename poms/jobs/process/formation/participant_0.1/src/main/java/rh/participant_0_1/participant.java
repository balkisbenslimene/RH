 ...
 * @status
 */
public class participant implements TalendJob {
	...
	private final String jobName = "participant";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(participant.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_RH_participant = new byte[0];
		static byte[] commonByteArray_RH_participant = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RH_participant.length) {
					if (length < 1024 && commonByteArray_RH_participant.length == 0) {
						commonByteArray_RH_participant = new byte[1024];
					} else {
						commonByteArray_RH_participant = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_RH_participant, 0, length);
				strReturn = new String(commonByteArray_RH_participant, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RH_participant) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:+lFMAFYYgTEWgfGA88/wXav9vaSpKWP2KNM9GQ==");
...
// Start of branch "row1"
							if (row1 != null) {
								...
								globalMap.put("row1.mle", row1.mle);

								globalMap.put("row1.prenom", row1.prenom);

								globalMap.put("row1.nom", row1.nom);

								globalMap.put("row1.Date_Debut", row1.Date_Debut);

								globalMap.put("row1.Date_Fin", row1.Date_Fin);

								globalMap.put("row1.nb_de_jours", row1.nb_de_jours);

								globalMap.put("row1.themes", row1.themes);

								globalMap.put("row1.organisme", row1.organisme);

								globalMap.put("row1.mode_de_formation", row1.mode_de_formation);

								globalMap.put("row1.Depenses", row1.Depenses);
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tFileInputExcel_2 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:NqdDTe64sa6H21qNl743vjHfDROt18WLxaOcnw==");
...
// Start of branch "row2"
							if (row2 != null) {
								...
								globalMap.put("row2.MATRICULE_PARTICIPANT", row2.MATRICULE_PARTICIPANT);

								globalMap.put("row2.NOM_ET_PRENOM", row2.NOM_ET_PRENOM);

								globalMap.put("row2.theme", row2.theme);

								globalMap.put("row2.AFFECTATION", row2.AFFECTATION);
	...
	public static void main(String[] args) {
		final participant participantClass = new participant();

		int exitCode = participantClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = participant.class.getClassLoader()
					.getResourceAsStream("rh/participant_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = participant.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : participant");
		}
...
/************************************************************************************************
 * 74848 characters generated by Talend Open Studio for Data Integration on the
 * 7 mars 2024 à 10:28:44 WAT
 ************************************************************************************************/